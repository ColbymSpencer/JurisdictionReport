...
```{r check_identical}
real <- read_excel("Jurisdiction Report Master Spreadsheet.xlsx", sheet = 5) |>
  janitor::clean_names() |>
  mutate(unit_name = str_replace_all(unit_name, "\r\n" ," "), 
         unit_name = str_squish(unit_name)) |>
  filter(owner != 'Total') |>
...
# parks in 'real' not in 'combined'
unique(real$site_name)[!unique(real$site_name) %in% unique(combined$site_name)]

# records in 'real' not in 'combined'
real |>
  anti_join(combined, join_by(site_name, lease_number))

# records in 'combined' not in 'real'
combined |>
  anti_join(real, join_by(site_name, lease_number))
```
